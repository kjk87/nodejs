...
export class LuckyDrawPurchaseService extends CoreService {
    ...
    public async getJoinCount(luckyDrawSeqNo:number, member:MemberA, manager?: EntityManager){
        
        let filter:LuckyDrawPurchaseFilter = {};
        filter.luckyDrawSeqNo = luckyDrawSeqNo;
        filter.userKey = member.userKey;

        return await this.luckyDrawPurchaseModel.getMyJoinCount(filter, manager);
    }
    ...
    public async purchase(req: Request, res: Response, luckyDrawPurchase:LuckyDrawPurchase, member:MemberA, manager?: EntityManager){
        ...
        let luckyDraw : LuckyDraw = await this.luckyDrawService.get(luckyDrawPurchase.luckyDrawSeqNo, manager);
        ...
        if(luckyDraw.engageNumber > 0){
            let joinCount = await this.getJoinCount(luckyDraw.seqNo, member, manager);
        ...
        if(luckyDrawPurchase.engagedPrice > 0){
            ...
            historyBall.subject = getLang('id').luckyDrawUseBallSubject;
            historyBall.comment = getLang('id').luckyDrawUseBallComment;
