...
import { DataSource, DeleteQueryBuilder, EntityManager, QueryRunner, SaveOptions, SelectQueryBuilder, Transaction, UpdateQueryBuilder } from "typeorm";
...
export class CoreModel<T extends CoreEntity> {
    constructor(
		...
		protected rpDataSource: DataSource,
	...
	public async get(id: any, entity: any = this.defaultEntity, filter?: any, manager?: EntityManager): Promise<T | undefined> {
        ...
        manager = manager ? manager : this.rpDataSource.createEntityManager();
	...
	public async getByFilter(filter?: any, order?: IOrder[], entity: any = this.defaultEntity, manager?: EntityManager): Promise<any> {
	...
	public async list(filter?: any, order?: IOrder[], paging: IPaging = { page: 1, limit: 10 }, entity: any = this.defaultEntity, manager?: EntityManager): Promise<CoreListResponse<any>> {
		...
		if(paging) {
			this.setPaging(builder, filter, paging);
		}
	...
	public async all(filter?: any, order?: IOrder[], entity: any = this.defaultEntity, manager?: EntityManager): Promise<any> {
		manager = manager ? manager : this.rpDataSource.createEntityManager();
	...
	// public async update(data: T, manager?: EntityManager, options?: SaveOptions): Promise<T> {
	// 	manager = manager ? manager : this.defaultDataSource.createEntityManager();
	// 	return await manager.save(data, options);
	// }
	...
	protected setOrder(builder: SelectQueryBuilder<any>, order: IOrder[], defaultTable: string = this.defaultAlias) {
		...
		for(let i = 0; i < order.length; ++i) {
			...
			if(order[i].column) {
				...
				if(order[i].custom){
					builder.addOrderBy(order[i].column, dir);
				}else{
	...
	/* paging */
	protected setPaging(builder: SelectQueryBuilder<any>, filter, paging: IPaging) {
		if(paging) {
			if(paging.start !== undefined) {
				builder.offset(paging.start);
				
			}
			else if(paging.page !== undefined) {
