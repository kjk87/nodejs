...
export class LotteryJoinModel extends CoreModel<LotteryJoin> {
     constructor() {
        super(BUFLEXZ_DATASOURCE, BUFLEXZ_RP_DATASOURCE, LotteryJoin);
    }
    ...
    public async procJoinMultiLottery(member: MemberA, lottery: Lottery, params: LotteryJoinCreateParams, manager?: EntityManager) {
    ...
    public async procJoinLottery(member: MemberA, lottery: Lottery, params: LotteryJoinCreateParams, number: number[], manager?: EntityManager) {
        manager = manager ? manager : BUFLEXZ_DATASOURCE.createEntityManager()
        let no = getRandomNum(1, 1, 28);
        let query1: string = '';
        let query2: string = '';
        ...
        let qy: string = "(";
        qy += "'" + (moment().format('YYYYMMDDHHmmssSSS') + getRandomNum(4, 0, 9) + 0) + "'";
        qy += ',';
        qy += "'" + member.userKey + "'";
        qy += ',';
        qy += lottery.lotteryRound;
